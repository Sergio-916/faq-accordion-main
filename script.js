...
let title = document.querySelectorAll('.header_content');
let textContent = document.querySelectorAll('.hidden_text');
let iconContent = document.querySelectorAll('.icon_content');


title.forEach((item, index) => {
    item.addEventListener('click', () => {
        if (textContent[index].classList == 'hidden_text'){
            textContent[index].setAttribute('class', 'show_text')
            iconContent[index].innerHTML = iconMinus;
           
        } else {
            textContent[index].setAttribute('class', 'hidden_text')
            iconContent[index].innerHTML = iconPlus;
